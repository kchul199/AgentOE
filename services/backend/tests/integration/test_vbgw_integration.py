...
def test_websocket_invalid_token_rejected(client):
    ...
    with pytest.raises(Exception), client.websocket_connect(url) as ws:
        ws.receive_text()


def test_websocket_missing_token_rejected(client):
    """token 파라미터 없음 → HTTP GET 으로 WS 엔드포인트 접근 시 404.
    (WS 전용 라우트는 HTTP GET 을 처리하지 않음)
    """
...
def test_lease_conflict_rejected(client, valid_token):
    """Lease Lock 획득 실패 → error 이벤트 (LEASE_CONFLICT) + 4002 close."""
    with patch(
        "app.api.v1.routers.vbgw.SessionRepository",
        ...
        with client.websocket_connect(url) as ws:
            data = json.loads(ws.receive_text())
            assert data["event"] == "error"
            assert data["code"] == "LEASE_CONFLICT"
...
def test_ended_session_rejected(client, valid_token):
    """ENDED 세션 재연결 → error 이벤트 (SESSION_ENDED) + 4004 close."""
    repo = _mock_session_repo()
    # restore_or_create_orchestrator 가 None 을 반환하려면 hot_state["status"] == "ENDED" 여야 함.
    # restore_hot_state 가 None 을 반환하면 신규 세션으로 처리되므로 ENDED dict 를 반환해야 한다.
    repo.restore_hot_state.return_value = {"status": "ENDED"}
    ...
    with patch("app.api.v1.routers.vbgw.SessionRepository", return_value=repo):
        ...
        with client.websocket_connect(url) as ws:
            data = json.loads(ws.receive_text())
            assert data["event"] == "error"
            assert data["code"] == "SESSION_ENDED"
